        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            while (true)
            {
                ...
                if (result.MessageType != NetWebSocketMessageType.Text)
                {
                    throw new OutOfProcessCapabilityException(
...
internal static class SidecarCapabilityErrors
{
    ...
    public const string RegistrationBusy = "sidecar_module_busy";
...
internal static class OutOfProcessCapabilitySecurity
{
        ...
        DateTimeOffset now)
    {
        ...
        return grant.IssuedAt <= now
            ...
            && string.Equals(grant.SourceId, SourceId, StringComparison.Ordinal)
            && string.Equals(authorization.SourceId, SourceId, StringComparison.Ordinal)
    ...
    public static string CreateStorageContinuationProof(
        SidecarHostEntryStorageContinuationAuthority authority,
        string controlToken)
    {
        ArgumentNullException.ThrowIfNull(authority);
        ArgumentException.ThrowIfNullOrWhiteSpace(controlToken);
        var value = "host-storage-continuation|"
    ...
    public static bool ValidateStorageContinuationProof(
        SidecarHostEntryStorageContinuationAuthority authority,
        string controlToken)
    {
        if (authority is null || string.IsNullOrWhiteSpace(authority.Proof))
            return false;
        var expected = CreateStorageContinuationProof(authority, controlToken);
        return CryptographicOperations.FixedTimeEquals(
            Encoding.UTF8.GetBytes(expected),
            Encoding.UTF8.GetBytes(authority.Proof));
    }
