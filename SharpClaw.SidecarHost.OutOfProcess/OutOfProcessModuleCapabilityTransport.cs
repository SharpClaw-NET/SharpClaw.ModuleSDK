...
internal sealed class OutOfProcessModuleCapabilityTransport : ISidecarCapabilityTransport, IAsyncDisposable
{
        ...
        Type terminalType)
    {
        ...
        var SourceId = Volatile.Read(ref _moduleId)
            ?? throw new InvalidOperationException(
        ...
        {
            if (!string.Equals(entry.OwnerId, SourceId, StringComparison.Ordinal)
    ...
    internal void SetAuthorization(SidecarHostAuthorization authorization)
    {
        ...
        lock (_sync)
        {
            if (_moduleId is null || _graphId is null)
                throw new InvalidOperationException(
                    "The module capability transport is not initialized.");
            if (!string.Equals(authorization.SourceId, _moduleId, StringComparison.Ordinal))
                throw new OutOfProcessCapabilityException(
        ...
        CancellationToken ct)
    {
        ...
        if (!string.Equals(binding.SourceId, _moduleId, StringComparison.Ordinal)
            || !string.Equals(binding.GraphId, _graphId, StringComparison.Ordinal)
            || binding.ProtocolVersion != OutOfProcessSidecarHostProtocol.Version)
        {
            throw new OutOfProcessCapabilityException(
                SidecarCapabilityErrors.Unauthorized,
...
internal sealed partial class OutOfProcessModuleCapabilityConnection : IAsyncDisposable
{
        ...
        CancellationTokenSource cancellation) : IDisposable
    {
        ...
        private int _terminalResponseSent;

        private int _actionResponseAccepted;
        ...
        public bool ResponseAcknowledged =>
            Volatile.Read(ref _terminalResponseSent) != 0
            && Volatile.Read(ref _actionResponseAccepted) != 0;

        public bool TerminalResponseSent => Volatile.Read(ref _terminalResponseSent) != 0;

        public bool ActionResponseAccepted => Volatile.Read(ref _actionResponseAccepted) != 0;

        public void MarkTerminalResponseSent() =>
            Interlocked.Exchange(ref _terminalResponseSent, 1);

        public void MarkActionResponseAccepted() =>
            Interlocked.Exchange(ref _actionResponseAccepted, 1);
        ...
        try
        {
            ...
            try
            {
                ...
                OutOfProcessProtocolTestFixture.RecordRebindState(
                    ...
                    $"call={request.Call.CallId:N};accepted={validation.Accepted};code={validation.Code}");
#endif
                ThrowIfRejected(validation);
                if (_incomingTerminals.TryGetValue(
                        request.Call.CallId,
                        out var incomingTerminal))
                {
                    incomingTerminal.MarkActionResponseAccepted();
#if OUT_OF_PROCESS_PROTOCOL_TEST_FIXTURE
                    OutOfProcessProtocolTestFixture.RecordRebindState(
        ...
        CancellationToken ct)
    {
        ...
        try
        {
                ...
                && request.CrossSidecarActionRequest is not null)
            {
                ...
                incoming.MarkTerminalResponseSent();
                return;
            }
        ...
        ThrowIfRejected(_session.RecordTerminal(
            ...
            request.Receipt));
        var activeCarrierId = validationRequest.NestedCarrier?.CarrierId
            ?? validationRequest.HostContext?.CapabilityId;
        using var carrierScope = activeCarrierId is { } carrierId
            ? _transport.PushActiveCarrier(carrierId, request.Call)
            : null;
        ...
        try
        {
                ...
                && request.EffectiveHostEntryContext is { } effectiveHostEntry)
            {
                var peerCall = request.Call with
                {
                    ...
                    SourceId = _session.Binding.SourceId,
        ...
        CancellationToken ct)
    {
        ...
        var authorityValid = ValidateTerminalAuthority(
                ...
                SidecarCapabilityTransportValidation.ComputeTerminalAuthorityBindingHash(
                    request.Authority))
            && request.Authority.SourceId == Binding.SourceId
    ...
    private static string DescribeCapabilityCall(SidecarCapabilityCallIdentity? call) =>
        call is null
            ...
            : $"session={call.SessionId};request={call.RequestId};cancel={call.CancellationId};"
                + $"call={call.CallId};nonce={call.ReplayNonce};module={call.SourceId};"
    ...
    private void RecordRebindState(string phase) =>
        OutOfProcessProtocolTestFixture.RecordRebindState(
            ...
            string.Join(
                ...
                DescribeIncomingTerminalAcknowledgments(),
                DescribeOutgoingReservations()));

    private string DescribeIncomingTerminalAcknowledgments() =>
        $"incomingTerminalAcknowledgments=[{string.Join(",", _incomingTerminals
            .OrderBy(entry => entry.Key)
            .Select(entry =>
                $"{entry.Key:N}:{entry.Value.TerminalResponseSent}:{entry.Value.ActionResponseAccepted}"))}]";
