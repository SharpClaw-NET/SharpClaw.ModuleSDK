...
public sealed class OutOfProcessRegistrationDiscovery : IAsyncDisposable
{
    private HttpClient? _httpClient;
    private readonly Uri _controlAddress;
    private readonly string _controlToken;

    internal OutOfProcessRegistrationDiscovery(
        Uri controlAddress,
        string controlToken,
    ...
    public IReadOnlyList<ScopedStorageContractDescriptor> StorageContracts { get; }

    public SidecarApplicationDiscovery Application { get; }

    public async Task<OutOfProcessRegistrationClient> AuthorizeAsync(
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            using var response = await http.PostAsJsonAsync(
                OutOfProcessSidecarHostProtocol.AuthorizationPath,
                decision,
                OutOfProcessProtocolCodec.JsonOptions,
                ct);
            response.EnsureSuccessStatusCode();
            return new OutOfProcessRegistrationClient(
                _controlAddress,
                _controlToken,
                http,
                Discovery,
...
public sealed class OutOfProcessRegistrationClient : IAsyncDisposable
{
    private readonly Uri _controlAddress;
    private readonly string _controlToken;
    ...
    internal OutOfProcessRegistrationClient(
        Uri controlAddress,
        string controlToken,
        HttpClient httpClient,
        SidecarDiscoveryEnvelope discovery,
    ...
    public static async Task<OutOfProcessRegistrationClient> CreateAuthorizedAsync(
        Uri controlAddress,
        string controlToken,
        SidecarHostDescriptorCatalog hostCatalog,
        CancellationToken ct = default)
    ...
    public static async Task<OutOfProcessRegistrationDiscovery> DiscoverAsync(
        Uri controlAddress,
        string controlToken,
        CancellationToken ct = default)
    {
        ...
        try
        {
            var document = await http.GetFromJsonAsync<SidecarDiscoveryDocument>(
                OutOfProcessSidecarHostProtocol.DiscoveryPath,
                OutOfProcessProtocolCodec.JsonOptions,
                ct)
                ?? throw new OutOfProcessProtocolException(
            ...
            return new OutOfProcessRegistrationDiscovery(
                controlAddress,
                controlToken,
                http,
                discovery,
        ...
        SidecarApplicationDiscovery application)
    {
        ...
        if (!string.Equals(application.SourceId, discovery.SourceId, StringComparison.Ordinal)
        ...
        if (application.ActionEntries.Any(entry => entry is null
                || !string.Equals(entry.SourceId, discovery.SourceId, StringComparison.Ordinal)
        ...
        CancellationToken ct = default)
    {
        ...
        if (!string.Equals(grant.SourceId, Discovery.SourceId, StringComparison.Ordinal))
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            using var response = await _httpClient.PostAsJsonAsync(
                OutOfProcessSidecarHostProtocol.ApplicationCliPath,
                request,
                OutOfProcessProtocolCodec.JsonOptions,
                ct);
    ...
    public async ValueTask<HttpEndpointResponse> InvokeEndpointAsync(
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            using var response = await _httpClient.PostAsync(
                OutOfProcessSidecarHostProtocol.ApplicationEndpointPath,
                content,
                ct);
            if (!response.IsSuccessStatusCode)
            {
            ...
            if (!string.Equals(result.SourceId, Discovery.SourceId, StringComparison.Ordinal)
    ...
    public async ValueTask<IActionOutcome<TResult>> InvokeRegistrationActionEntryAsync<TAction, TResult>(
        ActionDescriptor<TAction, TResult> descriptor,
        TAction action,
        HostActionEntryRequestContext hostActionContext,
        CancellationToken ct = default)
    {
        ...
        if (!hostActionContext.IsWellFormed(now)
            ...
            || hostActionContext.Ingress is not (
                ...
                or HostActionEntryIngress.CrossRegistration))
        {
            throw new OutOfProcessProtocolException(
                SidecarProtocolErrors.MalformedMessage,
        ...
        try
        {
            var outcome = await CapabilitySession.InvokeRegistrationActionEntryAsync(
                descriptor,
                action,
                identity,
                payload,
    ...
    public async ValueTask<IActionOutcome<JsonElement>> InvokeRegistrationActionEntryAsync(
        SidecarActionDefinition definition,
        SidecarActionDescriptorIdentity descriptor,
        JsonElement action,
        HostActionEntryRequestContext hostActionContext,
    ...
    private Uri ExchangeUri()
    {
        var builder = new UriBuilder(new Uri(
            _controlAddress,
            OutOfProcessSidecarHostProtocol.ExchangePath))
        {
    ...
    private Uri CapabilitiesUri()
    {
        var builder = new UriBuilder(new Uri(
            _controlAddress,
            OutOfProcessSidecarHostProtocol.CapabilityPath))
        {
            Scheme = string.Equals(_controlAddress.Scheme, Uri.UriSchemeHttps, StringComparison.Ordinal)
                ? "wss"
                : "ws",
