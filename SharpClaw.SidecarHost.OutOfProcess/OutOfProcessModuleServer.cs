...
public sealed class OutOfProcessModuleServer : IAsyncDisposable
{
    ...
    public static Task<OutOfProcessModuleServer> CreateAsync(
        ...
        CreateAsync(
            ReadRequiredEnvironment(
                OutOfProcessSidecarHostProtocol.RegistrationDirectoryEnvironmentVariable),
            new Uri(ReadRequiredEnvironment(
                OutOfProcessSidecarHostProtocol.ControlAddressEnvironmentVariable)),
            ReadRequiredEnvironment(
                OutOfProcessSidecarHostProtocol.ControlTokenEnvironmentVariable),
            args,
            ct);
    ...
    private void MapEndpoints()
    {
        ...
        _app.MapGet(OutOfProcessSidecarHostProtocol.ReadinessPath, () =>
            Results.Json(new
            {
                SourceId = _runtime.Graph.Identity.Id,
                authorized = Volatile.Read(ref _authorization) is not null,
            }, OutOfProcessProtocolCodec.JsonOptions));

        _app.MapGet(OutOfProcessSidecarHostProtocol.DiscoveryPath, () =>
        {
        ...
        _app.MapGet(
            OutOfProcessSidecarHostProtocol.ApplicationPath,
            () => Results.Json(
        ...
        _app.MapPost(OutOfProcessSidecarHostProtocol.AuthorizationPath, AuthorizeAsync);
        _app.MapGet(OutOfProcessSidecarHostProtocol.CapabilityPath, HandleCapabilitiesAsync);
        _app.MapGet(OutOfProcessSidecarHostProtocol.ExchangePath, HandleExchangeAsync);
    }
    ...
    private async Task<IResult> AuthorizeAsync(HttpContext context, CancellationToken ct)
    {
        ...
        if (!string.Equals(decision.SourceId, _runtime.Graph.Identity.Id, StringComparison.Ordinal)
            || !string.Equals(
                decision.Authorization.SourceId,
        ...
        CancellationToken ct)
    {
        ...
        if (!string.Equals(request.SourceId, _runtime.Graph.Identity.Id, StringComparison.Ordinal)
        ...
        try
        {
            var handler = ActivatorUtilities.GetServiceOrCreateInstance(
                ...
                contribution.HandlerType) as ICliHandler
        ...
        CancellationToken ct)
    {
        ...
        if (!request.IsWellFormed(DateTimeOffset.UtcNow)
            ...
            || !string.Equals(
                _runtime.Graph.Identity.Id,
                Volatile.Read(ref _authorization)?.SourceId,
                StringComparison.Ordinal))
        {
            return Results.BadRequest(new
            {
        ...
        try
        {
            ...
            if (request.Route.Transport == HostEndpointTransport.Http)
            {
                ...
                try
                {
                    handler = ActivatorUtilities.GetServiceOrCreateInstance(
                            scope.ServiceProvider,
                            endpoint.HandlerType) as IHttpEndpointHandler
                        ?? throw new InvalidOperationException(
                            $"Endpoint '{request.Route.HandlerIdentity}' does not implement "
                            + $"'{typeof(IHttpEndpointHandler).FullName}'.");
                }
            ...
            {
                IWebSocketEndpointHandler handler;
                try
                {
                    handler = ActivatorUtilities.GetServiceOrCreateInstance(
                            scope.ServiceProvider,
                            endpoint.HandlerType) as IWebSocketEndpointHandler
                        ?? throw new InvalidOperationException(
                            $"Endpoint '{request.Route.HandlerIdentity}' does not implement "
                            + $"'{typeof(IWebSocketEndpointHandler).FullName}'.");
                }
                catch (InvalidOperationException)
                {
                    return InvalidEndpointHandlerResult();
                ...
                await handler.InvokeAsync(
                    ...
                    scope.ServiceProvider.GetRequiredService<IHostActionEntry>(),
                    linked.Token);
                result = HttpEndpointResponse.Empty(StatusCodes.Status204NoContent);
            }
    ...
    private async Task HandleExchangeAsync(HttpContext context)
    {
        ...
        try
        {
            ...
            {
                await protocol.SendErrorAsync(
                    SidecarProtocolErrors.RegistrationBusy,
