...
public sealed class ApplicationSmokeModule : ISharpClawModule
{
    ...
    public static EndpointRouteDescriptor ApplicationEndpointRoute { get; } = new(
        ApplicationEndpointId,
        "/application/endpoint",
        "POST",
        HostEndpointTransport.Http);

    public static EndpointRouteDescriptor StorageHeavyEndpointRoute { get; } = new(
        StorageHeavyEndpointId,
        "/application/storage-heavy",
        "POST",
        HostEndpointTransport.Http);

    public static EndpointRouteDescriptor ScopedEndpointRoute { get; } = new(
        ScopedEndpointId,
        "/application/scoped",
        "GET",
        HostEndpointTransport.Http);
    ...
    public void ConfigureServices(IServiceCollection services)
    {
        ...
        services.AddAction(AgentsJobImportAction);
        services.AddAction(PermissionPolicyAction);
        services.AddScoped<ScopedEndpointResource>();
        services.AddScoped<ScopedTerminalResource>();
        ...
        services.OnAction(HostAction).Use<AuthorizationHook>(
            HostCapabilities,
            new HookOrdering(HostActionHookId));
        services.OnAction(ChildAction).Use<ChildAuthorizationHook>(
            HostCapabilities,
            new HookOrdering(ChildActionHookId));
        services.OnAction(OwnedAction).Use<AuthorizationHook>(
            HostCapabilities,
            new HookOrdering(OwnedActionHookId));
        services.OnAction(PermissionPolicyAction).Use<PermissionAuthorizationHook>(
            ActionInterceptionCapabilities.Inspect,
            new HookOrdering(PermissionPolicyActionHookId));
        services.AddActionEntry<AgentsJobImportAction, AgentsJobImportResult, AgentsJobImportTerminal>(
            AgentsJobImportAction,
            AgentsJobImportTerminalId);
        services.AddActionEntry<
        ...
        services.AddTool<HostEntryToolHandler>(new ToolDescriptor(
            HostEntryToolName,
        ...
        services.AddHttpEndpoint<ApplicationEndpoint>(ApplicationEndpointRoute);
        services.AddHttpEndpoint<StorageHeavyEndpoint>(StorageHeavyEndpointRoute);
        services.AddHttpEndpoint<ScopedEndpoint>(ScopedEndpointRoute);
        services.AddWebSocketEndpoint<EchoWebSocketEndpoint>(WebSocketEndpointRoute);
        services.AddCliCommand<ApplicationCliHandler>(new CliCommandDescriptor(
            CliName,
            ["app-inspect"],
        ...
        services.AddCliCommand<CapabilityCliHandler>(new CliCommandDescriptor(
            CapabilityCliName,
            ["app-capabilities"],
        ...
        services.AddCliCommand<HostEntryCliHandler>(new CliCommandDescriptor(
            HostEntryCliName,
            ["app-host-entry"],
        ...
        services.AddCliCommand<SelfOwnedEntryCliHandler>(new CliCommandDescriptor(
    ...
    public sealed class ApplicationCliHandler(
        ...
        ModuleContributionGraph graph) : ICliHandler
    {
    ...
    public sealed class ApplicationEndpoint : IHttpEndpointHandler
    {
    ...
    public sealed class StorageHeavyEndpoint(IScopedStorageGateway storage) : IHttpEndpointHandler
    {
        public async ValueTask<HttpEndpointResponse> InvokeAsync(
            HostEndpointRouteRequest request,
            IHostActionEntry hostActionEntry,
    ...
    public sealed class ScopedEndpoint(ScopedEndpointResource resource) : IHttpEndpointHandler
    {
        public ValueTask<HttpEndpointResponse> InvokeAsync(
            HostEndpointRouteRequest request,
            IHostActionEntry hostActionEntry,
            CancellationToken cancellationToken) =>
            ValueTask.FromResult(HttpEndpointResponse.Json(
                200,
                JsonSerializer.SerializeToElement(new { state = resource.State })));
    }

    public sealed class EchoWebSocketEndpoint : IWebSocketEndpointHandler
    {
    ...
    public sealed class HostEntryCliHandler(
        IHostActionEntry hostActionEntry,
        IScopedStorageGateway storage) : ICliHandler
    {
            ...
            CancellationToken ct)
        {
            try
            {
                ...
                var actionMode = invocation.Arguments.FirstOrDefault() switch
                {
                    ...
                    "terminal-storage" => "terminal-storage-root",
                    _ => "host-entry",
                };
                var outcome = await hostActionEntry.InvokeAsync<ApplicationSmokeAction, ApplicationSmokeResult>(
                    new HostActionEntryRequest<ApplicationSmokeAction, ApplicationSmokeResult>(
                        ...
                        hostActionContext),
                    new HostActionTerminal(storage),
                    ct);
            ...
            catch (ActionFailedException ex)
            {
                return new CliResult(
                    false,
                    [new CliOutput("stderr", $"{ex.GetType().FullName}: {ex.Message}")],
                    ex.Error);
            }
            catch (Exception ex)
            {
    ...
    public sealed class SelfOwnedEntryCliHandler(
        IHostActionEntry hostActionEntry,
        AgentsJobImportTerminal registeredTerminal) : ICliHandler
    {
        public async ValueTask<CliResult> ExecuteAsync(
            CliInvocation invocation,
            CancellationToken ct)
        ...
        private static CliResult CreateResult(
    ...
    public sealed class CapabilityCliHandler(
        ...
        IScopedStorageGateway storage,
        IActionDispatcher dispatcher) : ICliHandler
    {
        public async ValueTask<CliResult> ExecuteAsync(
            CliInvocation invocation,
            CancellationToken ct)
        {
            try
            {
                        ...
                        StringComparison.Ordinal))
                {
                    ...
                    return new CliResult(
                        true,
                        [new CliOutput("stdout", "storage-heavy:5")]);
                }
    ...
    private sealed class HostActionTerminal(
        IScopedStorageGateway? storage = null) :
        IHostActionEntryTerminal<ApplicationSmokeAction, ApplicationSmokeResult>
    {
        ...
        public async ValueTask<ApplicationSmokeResult> InvokeAsync(
            ...
            context.Action.Mode switch
            {
                ...
                "terminal-storage-root" => await InvokeStorageAsync(storage, context, ct),
        ...
        private static async ValueTask<ApplicationSmokeResult> InvokeStorageAsync(
            IScopedStorageGateway? storage,
            ActionContext<ApplicationSmokeAction> context,
            CancellationToken ct)
        {
            if (storage is null)
                throw new InvalidOperationException("The terminal has no storage gateway.");

            var result = await storage.InvokeAsync(
                Id,
                "application-store",
                "echo",
                JsonSerializer.SerializeToElement(new { value = context.Action.Value }),
                ct);
            return new ApplicationSmokeResult($"terminal-storage:{result.GetRawText()}");
        }
...
public sealed class CrossSidecarModule : ISharpClawModule
{
    ...
    {
        services.AddAction(OwnedAction);
        services.AddAction(PermissionAction);
        services.AddScoped<ApplicationSmokeModule.ScopedTerminalResource>();
        services.AddStorage(new ScopedStorageContractDescriptor(
            Id,
            "target-store",
            [new ScopedStorageOperationDescriptor("echo")],
            "Cross-sidecar target storage."));
        services.AddActionEntry<CrossSidecarAction, CrossSidecarResult, TargetTerminal>(
            OwnedAction,
            TerminalId);
        services.AddActionEntry<
            PermissionPolicyReadAction,
