...
public sealed class OutOfProcessCrossSidecarProtocolTests
{
    ...
    private OutOfProcessRegistrationClient _targetClient = null!;
    ...
    public async Task StartSidecars()
    {
        ...
        _targetClient = await OutOfProcessRegistrationClient.CreateAuthorizedAsync(
            _targetAddress,
            _targetToken,
            targetCatalog);
    ...
    public async Task HostDrivenChatEntriesAreNotCrossSidecarTargets()
    {
        ...
        await using var client = await OutOfProcessRegistrationClient.CreateAuthorizedAsync(
            address,
            token,
    ...
    public async Task CrossSidecarActionUsesTargetDescriptorAndTerminal()
    {
        ...
        _targetClient.Application.ActionEntries.Should().ContainSingle(entry =>
            entry.SourceId == CrossSidecarModule.Id
    ...
    public async Task CrossSidecarUnknownTargetDoesNotDispatchTarget()
    {
        ...
        var targetCatalog = new SidecarHostDescriptorCatalog(
            ...
            OutOfProcessSidecarHostProtocol.Version,
            new SidecarPayloadLimits());
        await using var unauthorizedTarget = await OutOfProcessRegistrationClient.CreateAuthorizedAsync(
            _targetAddress,
            _targetToken,
            targetCatalog);
    ...
    private async Task<(OutOfProcessRegistrationClient Client, CountingActionDispatcher Dispatcher)> CreateSourceClientAsync(
        OutOfProcessRegistrationClient target)
    {
        var client = await OutOfProcessRegistrationClient.CreateAuthorizedAsync(
            _sourceAddress,
            _sourceToken,
            new SidecarHostDescriptorCatalog(
                [
        ...
        Type entryType)
    {
        ...
        var identity = ((ISharpClawModule)Activator.CreateInstance(entryType)!).Identity;
        var displayName = identity.DisplayName;
        var toolPrefix = identity.ToolPrefix;
        var requestedHooks = entryType == typeof(ApplicationSmokeModule)
            ? """
              [
                {
    ...
    private static OutOfProcessCapabilityHostOptions CreateOptions(
        ...
        IScopedStorageGateway? storageGateway = null,
    ...
    private sealed class EmptyStorageGateway : IScopedStorageGateway
    {
    ...
    private sealed class BlockingStorageGateway : IScopedStorageGateway
    {
