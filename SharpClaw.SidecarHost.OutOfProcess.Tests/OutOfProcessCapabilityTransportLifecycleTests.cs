...
public sealed class OutOfProcessCapabilityTransportLifecycleTests
{
    ...
    public async Task StartServer()
    {
        ...
        await File.WriteAllTextAsync(
            ...
            $$"""
            {
              ...
              "runtime": "dotnet",
              "hostMode": "sidecar",
              "entryType": "{{typeof(LifecycleSmokeModule).FullName}}",
              "requestedHooks": [
                {
                  "target": "host.smoke",
    ...
    private async Task<OutOfProcessRegistrationClient> CreateClientAsync()
    {
        ...
        return await OutOfProcessRegistrationClient.CreateAuthorizedAsync(
            _controlAddress,
            _controlToken,
            catalog);
    }
    ...
    private sealed class NoOpStorageGateway : IScopedStorageGateway
    {
        ...
        public Task<ScopedStorageClaimResult<T>> ClaimAsync<T>(
            ...
            CancellationToken cancellationToken = default) =>
            Task.FromException<ScopedStorageClaimResult<T>>(
                new InvalidOperationException("The lifecycle test storage gateway must not execute."));
        ...
        public Task<ScopedStorageMutationAndOutboxResult> CommitMutationAndOutboxAsync(
            ...
            CancellationToken cancellationToken = default) =>
            Task.FromException<ScopedStorageMutationAndOutboxResult>(
                new InvalidOperationException("The lifecycle test storage gateway must not execute."));
        ...
        public Task<ScopedStorageClaimRenewalResult> RenewClaimAsync(
            ...
            CancellationToken cancellationToken = default) =>
            Task.FromException<ScopedStorageClaimRenewalResult>(
                new InvalidOperationException("The lifecycle test storage gateway must not execute."));
        ...
        public Task<ScopedStorageClaimRecoveryResult> RecoverClaimAsync(
            ...
            CancellationToken cancellationToken = default) =>
            Task.FromException<ScopedStorageClaimRecoveryResult>(
                new InvalidOperationException("The lifecycle test storage gateway must not execute."));
    }
}
