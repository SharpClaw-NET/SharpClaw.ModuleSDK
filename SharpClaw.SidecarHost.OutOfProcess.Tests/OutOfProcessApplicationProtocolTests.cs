...
public sealed class OutOfProcessApplicationProtocolTests
{
    ...
    public async Task StartServer()
    {
        ...
        await File.WriteAllTextAsync(
            Path.Combine(_moduleDirectory, "package.json"),
            $$"""
            {
              ...
              "runtime": "dotnet",
              "hostMode": "sidecar",
              "entryType": "{{typeof(ApplicationSmokeModule).FullName}}",
              "requestedHooks": [
                {
    ...
    public async Task ProxyCompilesTypedSidecarHooksOnlyInsideTheOwningSidecar()
    {
        ...
        await using var proxy = new OutOfProcessRegistrationProxy(
            client.Discovery.SourceId,
            "Application Smoke",
            "appsmoke",
            client);
        var services = new ServiceCollection();
        foreach (var descriptor in proxy.GetServiceDescriptors())
            ((ICollection<ServiceDescriptor>)services).Add(descriptor);
        using var provider = services.BuildServiceProvider();
        var graph = new KernelGraphBuilder().Compile(provider);

        provider.GetServices<IExternalBehaviorAuthority>().Should().ContainSingle(authority =>
            authority.SourceId == client.Discovery.SourceId);
        graph.Services.Storage.Should().ContainSingle(storage =>
            storage.SourceId == client.Discovery.SourceId
    ...
    public async Task ApplicationDiscoveryAndCliUseTheSameModuleGraph()
    {
        ...
        client.Application.SourceId.Should().Be(client.Discovery.SourceId);
        ...
        client.StorageContracts.Should().ContainSingle(contract =>
            contract.SourceId == ApplicationSmokeModule.Id
        ...
        result.SourceId.Should().Be(client.Discovery.SourceId);
    ...
    public async Task RealCoreDispatcherExecutesExternalEndpointAndTypedEntryThroughSessionVerifier()
    {
        ...
        var typed = await client.InvokeRegistrationActionEntryAsync(
    ...
    public async Task RealCoreDispatcherExecutesDiscoveredEntryAndRejectsReplay()
    {
        ...
        var replay = async () => await client.InvokeRegistrationActionEntryAsync(
            definition,
            entry.Descriptor,
            action,
            context);
        ...
        var later = await client.InvokeRegistrationActionEntryAsync(
            definition,
            entry.Descriptor,
            laterAction,
            client.IssueHostActionContext(
    ...
    public async Task DiscoveredEntryRejectsMutationsAndCancellationBeforeTerminal()
    {
        ...
        var originalContext = client.IssueHostActionContext(
            ...
            Guid.NewGuid(),
            DateTimeOffset.UtcNow.AddMinutes(1));
        var changedPayload = async () => await client.InvokeRegistrationActionEntryAsync(
            definition,
            entry.Descriptor,
        ...
        cancellation.Cancel();
        var cancelled = async () => await client.InvokeRegistrationActionEntryAsync(
            definition,
            entry.Descriptor,
            cancelledAction,
            cancelledContext,
    ...
    [Test, CancelAfter(30000)]
    public async Task HostEntryTerminalCanUseStorageWhileBindingRotates()
    {
        await using var client = await CreateClientAsync();
        var storage = new CountingStorageGateway();
        var dispatcher = new CountingActionDispatcher();
        ...
        var result = await client.InvokeCliAsync(
            ...
            ["terminal-storage", "during-rotation"],
            IssueHostEntryContext(
                client,
                ApplicationSmokeModule.HostEntryCliName,
                DateTimeOffset.UtcNow.AddMinutes(1),
                "terminal-storage-root"));

        result.Result.Succeeded.Should().BeTrue(
            $"Host entry failed with {result.Result.Error?.Code}: {result.Result.Error?.Message}; "
            + string.Join(" | ", result.Result.Output.Select(item => item.Text)));
        result.Result.Output.Single().Text.Should().Contain(
            "host-entry:Completed:terminal-storage:");
    ...
    public async Task EndpointActivationUsesOneInvocationScope()
    {
        ...
        var endpointTypeName = ApplicationSmokeModule.ScopedEndpointId;
        var context = client.IssueHostActionContext(
            HostActionEntryIngress.Endpoint,
            endpointTypeName,
            client.Discovery.SourceId,
        ...
        var response = await client.InvokeEndpointAsync(CreateEndpointRequest(
            ApplicationSmokeModule.ScopedEndpointRoute,
            context));

        response.StatusCode.Should().Be(200);
        ReadJsonResponse(response).GetProperty("state").GetString().Should().Be("active");
        File.Exists(_scopeProbePath).Should().BeTrue();
        File.ReadAllText(_scopeProbePath).Should().Be("disposed");
    }

    [Test, CancelAfter(30000)]
    public async Task WebSocketEndpointUsesOneCapabilitySessionAndRejectsReplay()
    {
        await using var client = await CreateClientAsync();
        var storage = new CountingStorageGateway();
        var dispatcher = new CountingActionDispatcher();
        ...
        var request = CreateEndpointRequest(
            ApplicationSmokeModule.WebSocketEndpointRoute,
            client.IssueHostActionContext(
                HostActionEntryIngress.Endpoint,
                ApplicationSmokeModule.WebSocketEndpointId,
                client.Discovery.SourceId,
                ApplicationSmokeModule.HostAction,
                new ApplicationSmokeAction("websocket", "echo"),
                ApplicationSmokeModule.HostEntryCaller,
                ApplicationSmokeModule.HostEntryFeatures,
                Guid.NewGuid(),
                Guid.NewGuid(),
        ...
        var text = new WebSocketMessage(
            ...
            "sidecar-echo"u8.ToArray());
        var channel = new RecordingWebSocketChannel();
        channel.Enqueue(text);
        channel.Enqueue(new WebSocketMessage(
            WebSocketMessageType.Close,
            [],
            1000,
            "complete"));

        await client.InvokeWebSocketEndpointAsync(request, channel);

        var sent = channel.Sent.Should().ContainSingle().Which;
        sent.Type.Should().Be(text.Type);
        sent.Payload.Should().Equal(text.Payload);
        channel.CloseStatus.Should().Be(1000);
        channel.CloseDescription.Should().Be("complete");

        var replayChannel = new RecordingWebSocketChannel();
        replayChannel.Enqueue(new WebSocketMessage(
            WebSocketMessageType.Close,
            [],
            1000,
            "replay"));
        var replay = async () => await client.InvokeWebSocketEndpointAsync(
            request,
            replayChannel);
        (await replay.Should().ThrowAsync<OutOfProcessCapabilityException>())
            .Which.Code.Should().Be(SidecarCapabilityErrors.Replay);
        replayChannel.ReceiveCalls.Should().Be(0);
        ...
        var later = await client.InvokeCliAsync(
            ...
            IssueCliContext(
                ...
                "websocket-later-use"));
        later.Result.Succeeded.Should().BeTrue(
            $"CLI error {later.Result.Error?.Code}: {later.Result.Error?.Message}");
    }

    [Test, CancelAfter(30000)]
    public async Task WebSocketEndpointRejectsChangedRouteBeforeChannelUse()
    {
        await using var client = await CreateClientAsync();
        var descriptors = new OutOfProcessActionDescriptorCatalog();
        descriptors.Add(ApplicationSmokeModule.HostAction);
        ...
        var request = CreateEndpointRequest(
            ...
            client.IssueHostActionContext(
                ...
                client.Discovery.SourceId,
                ApplicationSmokeModule.HostAction,
                new ApplicationSmokeAction("websocket", "changed-route"),
                ApplicationSmokeModule.HostEntryCaller,
                ApplicationSmokeModule.HostEntryFeatures,
                Guid.NewGuid(),
                Guid.NewGuid(),
        ...
        {
            Route = ApplicationSmokeModule.WebSocketEndpointRoute.ToRouteIdentity() with
            {
                Path = "/application/changed-websocket",
            },
        };

        var act = async () => await client.InvokeWebSocketEndpointAsync(request, channel);

        (await act.Should().ThrowAsync<OutOfProcessProtocolException>())
            .Which.Code.Should().Be(SidecarProtocolErrors.UnknownHostDescriptor);
        channel.ReceiveCalls.Should().Be(0);
        channel.Sent.Should().BeEmpty();
    }

    [Test, CancelAfter(30000)]
    public async Task WebSocketEndpointCancellationReleasesTheRouteAndPreservesLaterUse()
    {
        await using var client = await CreateClientAsync();
        var storage = new CountingStorageGateway();
        var dispatcher = new CountingActionDispatcher();
        ...
        var channel = new RecordingWebSocketChannel();
        using var cancellation = new CancellationTokenSource();
        var invocation = client.InvokeWebSocketEndpointAsync(
            CreateEndpointRequest(
                ...
                client.IssueHostActionContext(
                    ...
                    client.Discovery.SourceId,
                    ApplicationSmokeModule.HostAction,
                    new ApplicationSmokeAction("websocket", "cancel"),
                    ApplicationSmokeModule.HostEntryCaller,
                    ApplicationSmokeModule.HostEntryFeatures,
                    Guid.NewGuid(),
                    Guid.NewGuid(),
        ...
        await channel.ReceiveStarted.Task.WaitAsync(TimeSpan.FromSeconds(5));
    ...
    public async Task EndpointAndTypedModuleActionEntryUseTheAuthenticatedCapabilitySession()
    {
        ...
        ReadJsonResponse(endpoint).GetProperty("value").GetString().Should().Be("entry-terminal:action");
        ...
        var importContext = client.IssueHostActionContext(
            ...
            Guid.NewGuid(),
            DateTimeOffset.UtcNow.AddMinutes(1));
        var import = await client.InvokeRegistrationActionEntryAsync(
            ApplicationSmokeModule.AgentsJobImportAction,
            importAction,
            importContext);
    ...
    public async Task MutatedHostSnapshotIsRejectedByDispatcherAuthorityBeforeTerminal()
    {
        ...
        var result = await client.InvokeRegistrationActionEntryAsync(
            ApplicationSmokeModule.AgentsJobImportAction,
    ...
    public async Task DisconnectedHostActionCompletionReleasesOutgoingCallBeforeCarrierCleanup()
    {
        ...
        var action = new AgentsJobImportAction("storage");
        var invocation = client.InvokeRegistrationActionEntryAsync(
            ApplicationSmokeModule.AgentsJobImportAction,
            action,
            client.IssueHostActionContext(
                HostActionEntryIngress.Cli,
    ...
    public async Task NonRetryableCompletionReleasesLiveSessionBeforeCarrierCleanup()
    {
        ...
        try
        {
            ...
            try
            {
                await client.InvokeRegistrationActionEntryAsync(
                    ApplicationSmokeModule.AgentsJobImportAction,
        ...
        var valid = await client.InvokeRegistrationActionEntryAsync(
            ApplicationSmokeModule.AgentsJobImportAction,
        ...
        for (var i = 0; i < 5; i++)
        {
            ...
            var followUp = await client.InvokeRegistrationActionEntryAsync(
                ApplicationSmokeModule.AgentsJobImportAction,
                followUpAction,
                client.IssueHostActionContext(
                    HostActionEntryIngress.Cli,
    ...
    public async Task StorageHeavyCliThenEndpointRotatesBeforeTheNextWorkflow()
    {
        ...
        var endpointTask = Task.Run(async () => await client.InvokeEndpointAsync(CreateEndpointRequest(
            ApplicationSmokeModule.StorageHeavyEndpointRoute,
            client.IssueHostActionContext(
                HostActionEntryIngress.Endpoint,
                ApplicationSmokeModule.StorageHeavyEndpointId,
                client.Discovery.SourceId,
                ApplicationSmokeModule.HostAction,
        ...
        endpoint.StatusCode.Should().Be(200);
        var endpointPayload = ReadJsonResponse(endpoint);
        endpointPayload.GetProperty("storageReads").GetInt32().Should().Be(3);
        endpointPayload.GetProperty("outcome").GetString().Should().Be(
            ActionOutcomeKind.Completed.ToString());
        endpointPayload.GetProperty("value").GetString().Should().Be("entry-terminal:endpoint");
        storage.InvokeCalls.Should().Be(8);
        dispatcher.RunCalls.Should().Be(1);
        dispatcher.TerminalCalls.Should().Be(1);
    ...
    [Test, CancelAfter(30000)]
    public async Task RebindDoesNotWaitForValidatedPostTerminalFailureCleanup()
    {
        ...
        try
        {
            ...
            dispatcher.FailAfterTerminal = true;
            ...
            cli.Result.Succeeded.Should().BeFalse();
            cli.Result.Error?.Code.Should().Be("post_terminal_denied");
            storage.InvokeCalls.Should().Be(4);
            dispatcher.RunCalls.Should().Be(1);
            dispatcher.TerminalCalls.Should().Be(1);
        }
    ...
    public async Task RebindReaderProcessesActionResponseBeforeBindingRotation()
    {
        ...
        try
        {
            ...
            endpointPayload.GetProperty("outcome").GetString().Should().Be(
                ActionOutcomeKind.Completed.ToString());
            endpointPayload.GetProperty("value").GetString().Should().Be(
    ...
    public async Task ActiveHostActionCarrierSurvivesBindingRotation()
    {
        ...
        preserved!.SourceId.Should().Be(issuedAuthority.SourceId);
    ...
    public async Task ToolHostActionEntryCarriesIssuedContextAndRejectsReplay()
    {
        ...
        start.HostActionContext.Contribution.IngressBinding.SecondaryIdentity.Should().Be(
            conversationId.ToString("D"));
        start.ConversationId.Should().Be(conversationId);
        ...
        tool.Content.Should().Contain($"conversation={conversationId:D}");
    ...
    [Test, CancelAfter(15000)]
    public async Task ToolHostActionEntryPreservesNullConversationIdentity()
    {
        await using var client = await CreateClientAsync();
        var storage = new CountingStorageGateway();
        var dispatcher = new CountingActionDispatcher();
        ...
        var context = client.IssueHostActionContext(
            ...
            null,
            ApplicationSmokeModule.HostAction,
            new ApplicationSmokeAction("host-tool", "tool-stateless"),
            ApplicationSmokeModule.HostEntryCaller,
            ApplicationSmokeModule.HostEntryFeatures,
            Guid.NewGuid(),
            Guid.NewGuid(),
        ...
        start.ConversationId.Should().BeNull();
        start.HostActionContext!.Contribution!.IngressBinding.SecondaryIdentity.Should().BeNull();
        var result = await client.InvokeToolAsync(start);
        var tool = result.Result.Deserialize<ToolResult>(OutOfProcessProtocolCodec.JsonOptions)!;
        tool.Content.Should().Contain("entry-terminal:tool-stateless")
            .And.Contain("conversation=null");
        dispatcher.RunCalls.Should().Be(1);
        dispatcher.TerminalCalls.Should().Be(1);
        storage.InvokeCalls.Should().Be(0);
    }
    ...
    [TestCase("changed")]
    [CancelAfter(15000)]
    public async Task ToolHostActionEntryRejectsConversationIdentityBeforeDispatch(string mutation)
    {
        await using var client = await CreateClientAsync();
        var storage = new CountingStorageGateway();
        var dispatcher = new CountingActionDispatcher();
        ...
        var context = client.IssueHostActionContext(
            ...
            conversationId.ToString("D"),
            ApplicationSmokeModule.HostAction,
            new ApplicationSmokeAction("host-tool", "conversation-mutation"),
            ApplicationSmokeModule.HostEntryCaller,
            ApplicationSmokeModule.HostEntryFeatures,
            Guid.NewGuid(),
            Guid.NewGuid(),
            deadline,
            invocationId);
        var invalidConversationId = mutation == "empty" ? Guid.Empty : Guid.NewGuid();
        var start = CreateHostEntryToolStart(
            client,
            definition,
            invocationId,
        ...
        start.ConversationId.Should().Be(invalidConversationId);
        start.HostActionContext!.Contribution!.IngressBinding.SecondaryIdentity.Should().Be(
            conversationId.ToString("D"));
        ...
        var laterInvocationId = Guid.NewGuid();
        var laterContext = client.IssueHostActionContext(
            HostActionEntryIngress.Tool,
            definition.ToolName,
            conversationId.ToString("D"),
            ApplicationSmokeModule.HostAction,
            new ApplicationSmokeAction("host-tool", "conversation-mutation-later"),
            ApplicationSmokeModule.HostEntryCaller,
            ApplicationSmokeModule.HostEntryFeatures,
            Guid.NewGuid(),
            Guid.NewGuid(),
            deadline,
            laterInvocationId);
        var later = await client.InvokeToolAsync(
            CreateHostEntryToolStart(
                client,
                definition,
                laterInvocationId,
                deadline,
                ...
                ApplicationSmokeModule.HostEntryCaller,
                conversationId,
                "conversation-mutation-later"));
        later.Result.Deserialize<ToolResult>(OutOfProcessProtocolCodec.JsonOptions)!
            .Content.Should().Contain("entry-terminal:conversation-mutation-later")
            .And.Contain($"conversation={conversationId:D}");
    }
    ...
    public async Task OversizedFullDiscoveryIsRejectedBeforeGrantExtraction()
    {
        ...
        AssertDiscoveryRejected(discovery, SidecarProtocolErrors.PayloadTooLarge);
    }
    ...
    private static HostEndpointRouteRequest CreateEndpointRequest(
        ...
        byte[]? body = null)
    {
        var routeContext = OutOfProcessHostActionEntryContextRegistry.WithoutPayloadBinding(context);
    ...
    private static JsonElement ReadJsonResponse(HttpEndpointResponse response)
    {
    ...
    private sealed class RecordingWebSocketChannel : IWebSocketChannel
    {
        private readonly Channel<WebSocketMessage> _incoming =
            Channel.CreateUnbounded<WebSocketMessage>(
        ...
        public ConcurrentQueue<WebSocketMessage> Sent { get; } = new();
        ...
        public void Enqueue(WebSocketMessage message)
        {
            if (!_incoming.Writer.TryWrite(message))
                throw new InvalidOperationException("The test WebSocket input is closed.");
        }
    ...
    private sealed class CountingStorageGateway : IScopedStorageGateway
    {
        ...
        public IReadOnlyList<ScopedStorageContractDescriptor> ListContracts()
        {
            ...
            return
            [
                new ScopedStorageContractDescriptor(
                    ApplicationSmokeModule.Id,
                    "application-store",
                    [new ScopedStorageOperationDescriptor("echo")]),
            ];
        }
        ...
        {
            InvokeCalls++;
            SourceId.Should().Be(ApplicationSmokeModule.Id);
    ...
    private sealed class CountingActionDispatcher : IActionDispatcher
    {
        ...
        public bool FailAfterTerminal { get; set; }
            ...
            CancellationToken ct)
        {
            ...
            var result = await terminal(
                ...
                ct);
            TerminalCalls++;
            if (FailAfterTerminal)
            {
                return new CountingFailedActionOutcome<TResult>(
                    new ExecutionError(
                        "post_terminal_denied",
                        "The test restriction denied the completed terminal."));
            }
    ...
    private sealed class CountingFailedActionOutcome<TResult>(ExecutionError error)
        : IActionOutcome<TResult>
    {
